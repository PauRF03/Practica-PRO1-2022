...
class BinaryTree {
  ...
  public:
	...
	std::istream& operator>>(std::istream &is, BinaryTree<T> &t) {  
		...
		while (size > 0) {
			T node;
			int numchildren; // 0: zero, -1: only left, 1: only right, 2: both
			is >> node >> numchildren;
			if (numchildren == 0) s.push(BinaryTree<T>(node, BinaryTree<T>(), BinaryTree<T>()));
